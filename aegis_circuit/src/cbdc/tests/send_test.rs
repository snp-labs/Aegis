mod send_tests {
    ...
    lazy_static! {
        pub static ref PK_FILE: String = "send.pk.dat".to_string();
        pub static ref VK_FILE: String = "send.vk.dat".to_string();
        pub static ref PK_UNCOMP_FILE: String = "send.pk.uncompressed.dat".to_string();
        pub static ref VK_UNCOMP_FILE: String = "send.vk.uncompressed.dat".to_string();
    ...
    fn test_constraints() {
        ...
        let test_input =
            <SendCircuit<C, GG> as MockingCircuit<C, GG>>::generate_circuit(33)
                .unwrap();
    ...
    fn test_make_pk_vk_size() {
        ...
        println!("Generate send circuit test input!");

        let test_input =
            <SendCircuit<C, GG> as MockingCircuit<C, GG>>::generate_circuit(TREE_HEIGHT)
                .unwrap();
    ...
    fn test_setup_and_proving_time() {
        let mut rng = ark_std::rand::rngs::StdRng::seed_from_u64(1 as u64);
        ...
        save_json_to_file(&vk_for_smart_contract.to_string(), &format!("{}{}", path, "send/send.vk.json"));
        ...
        image.append(&mut vec![
            ...
            test_input.instance.cm_v.clone().unwrap(),
            test_input.instance.rt.clone().unwrap(),
            test_input.instance.tag.clone().unwrap(),
        ...
        let proof_for_smart_contract: MyProof<ark_ec::bn::Bn<ark_bn254::Config>> = MyProof(proof.clone());
        save_json_to_file(&proof_for_smart_contract.to_string(), &format!("{}{}", path, "send/send.proof.json"));
