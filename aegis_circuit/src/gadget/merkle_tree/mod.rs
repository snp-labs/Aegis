...
pub trait Config {
    type Leaf: ?Sized + Debug; // merkle tree does not store the leaf
                               // leaf layer
...
impl<P: Config> Path<P> {
    ...
    pub fn verify<L: Borrow<P::Leaf>>(
        ...
        let (left_child, right_child) =
            select_left_right_child(self.leaf_index, &claimed_leaf_hash, &self.leaf_sibling_hash).unwrap();
...
impl<P: Config> MerkleTree<P> {
    ...
    pub fn new<L: Borrow<P::Leaf>>(
        ...
        for leaf in leaves.into_iter() {
            leaves_digests.push(P::LeafHash::evaluate(leaf_hash_param, leaf).unwrap())
        }
    ...
    fn updated_path<T: Borrow<P::Leaf>>(
        ...
        let new_leaf_hash: P::LeafDigest = P::LeafHash::evaluate(&self.leaf_hash_param, new_leaf).unwrap();
        ...
        while !is_root(prev_index) {
            ...
            let evaluated =
                P::TwoToOneHash::compress(&self.two_to_one_hash_param, left_child, right_child).unwrap();
...
mod tests {
    ...
    // A dummy config to use for testing purposes
    struct FieldMTConfig;

    impl Config for FieldMTConfig {
        type Leaf = [F]; // Change Leaf type to a slice of F
        type LeafDigest = F;
    ...
    #[test]
    fn test_merkle_tree_creation_and_verification() {
        // Initialize dummy parameters for the hash functions
        let leaf_hash_param = get_poseidon_params();
        let two_to_one_hash_param = get_poseidon_params();

        // let num_of_leaves: u64 = 1 << 1;
        ...
        let zero_leaf: F = F::from(BigUint::parse_bytes(b"0", 10).unwrap());
        // println!("Leaf: {:?}", zero_leaf);
        println!("Leaf: {:?}", leaf.0.to_string());
        let leaves = vec![[leaf.clone()], [zero_leaf.clone()]];
        // Build the Merkle tree
        // let mut leaves: Vec<Vec<F>> = Vec::with_capacity(num_of_leaves.try_into().unwrap());
        let merkle_tree = MerkleTree::<FieldMTConfig>::new(
            &leaf_hash_param,
            &two_to_one_hash_param,
            leaves.clone(),
        )
        ...
        println!("helght: {:#?}", merkle_tree.height);
        ...
        // Verify the proof for the first leaf
        let verification_result = proof
            .verify(&leaf_hash_param, &two_to_one_hash_param, &root, leaves[0])
            .expect("Verification failed");

        assert!(verification_result, "Merkle proof verification failed");
    }
}
