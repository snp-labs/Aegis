...
/// default value.
abstract contract BaseMerkleTree is Initializable {
    ...
    function __BaseMerkleTree_init(uint256 depth) internal onlyInitializing {
        __BaseMerkleTree_init_unchained(depth);
    }

    function __BaseMerkleTree_init_unchained(uint256 depth) internal onlyInitializing {
        _DEPTH = depth;
    ...
    function _initDefaultValues() private {
        ...
        for(uint i=0; i<_DEPTH; i++) {
            _defaultValues[i] = _DEFAULT_NODE_VALUE;
        }
    ...
    function _insert(bytes32 commitment) internal {
        ...
        _nodes[next_entry_idx] = hashed_commitment;
    }

    function _insert_two_element(bytes32 cm1, bytes32 cm2) internal {
        require(
            _numLeaves < _MAX_NUM_LEAVES,
            "BaseMerkleTree: Cannot append anymore"
        );

        bytes32 hashed_commitment = _hash(cm1, cm2);
    ...
    function _get_root() public view returns (bytes32) {
        return _nodes[0];
    }

    function _get_num_leaves() public view returns (uint256) {
        return _numLeaves;
    }
}